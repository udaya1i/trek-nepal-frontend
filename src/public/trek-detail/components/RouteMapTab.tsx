...
import { RoutePoint, ElevationPoint, Checkpoint } from "../types";
...
const RouteMapTab = ({ routePoints, elevationProfile }: RouteMapTabProps) => {
  ...
  const routePointsWithCheckpoints = routePoints.filter(point => point.checkpoint);
      ...
      <div className="space-y-6">
        ...
        {mapView === "map" ? (
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            ...
            {routePointsWithCheckpoints.length > 0 && (
              <div className="p-6 border-t border-border bg-blue-50/50">
                <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  <Icon name="MapPin" size={20} className="text-primary" />
                  Major Checkpoints on Route
                </h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {routePointsWithCheckpoints.map((point) => (
                    point.checkpoint && (
                      <CheckpointCard
                        key={point.checkpoint.id}
                        checkpoint={point.checkpoint}
                        isOvernight={true}
                        onClick={() => setSelectedCheckpoint(point.checkpoint!)}
                      />
                    )
                  ))}
                </div>
